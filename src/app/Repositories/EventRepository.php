...
class EventRepository
{
    ...
    public static function getSessions(TimeRangeInfo $timeRangeInfo, Domain $domain) {
        $sessions = DB::select(
            DB::raw("
                SELECT visitor_id, json_agg(json_build_object('visitor_id', visitor_id, 'referrer', referrer, 'created_at', created_at, 'event_name', event_name, 'location_href', location_href, 'country', country, 'language', language, 'device', device, 'os', os, 'browser', browser, 'time_on_page_seconds', time_on_page_seconds )) AS data
