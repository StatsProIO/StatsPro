...
Route::middleware(['auth', 'require_one_domain'])->group(function () {
    ...
    Route::get('/audience/{domain?}', function ($domain = null) {
        if($domain === null) {
            $firstDomain = Domain::where('user_id', Auth::user()->id)->oldest()->first();
            return redirect('/audience/'. $firstDomain->domain_name);
        }
    ...
    Route::get('/acquisition/{domain?}', function ($domain = null) {
        if($domain === null) {
            $firstDomain = Domain::where('user_id', Auth::user()->id)->oldest()->first();
            return redirect('/acquisition/'. $firstDomain->domain_name);
        }
